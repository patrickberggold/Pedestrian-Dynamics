...
from src.data_loader_direct import get_dataloader_direct
...
class trainer(object):
    def __init__(self, args, full_dataset):
        ...
        for set_name in ['train', 'valid', 'test']:
            self.data_loaders[set_name] = get_dataloader(args, set_name=set_name) if args.data_loading=='pickle' else get_dataloader_direct(args, set_name=set_name, full_dataset=full_dataset)
        # initialize device
    ...
    def _initialize_network(self, model_name: str):
        ...
        if model_name == 'SAR':
            ...
            net = SAR(self.args, self.device, full_dataset=self.full_dataset)
        elif model_name == 'Goal_SAR':
            from src.models.goal_sar import Goal_SAR
            net = Goal_SAR(self.args, self.device, full_dataset=self.full_dataset)
        else:
    ...
    def _train_epoch(self, epoch):
        ...
        # Progress bar
        train_bar = tqdm(self.data_loaders['train'], desc=f'Epoch {epoch}. Train batches')
        ...
        for batch_data in train_bar:
            ...
            if self.args.data_loading == 'pickle':
                ...
                inputs = {k: v.squeeze(0).float().to(self.args.device) for k, v in batch_data.items() if k != 'scene_id'}
                ground_truth, seq_list = inputs['abs_pixel_coord'], inputs['seq_list']
                    # batch_data['abs_pixel_coord'].to(self.args.device), \
                    # batch_data['seq_list'].to(self.args.device)
                inputs['scene'] = train_bar.iterable.dataset.scenes[batch_data['scene_id'].item()]
                batch_id = None
                del batch_data

            self.optimizer.zero_grad()  # sets grads to zero

            # Forward network, if_test for inference purposes (turn off teacher forcing)
    ...
    def _evaluate_epoch(self, epoch, mode='valid'):
        ...
        # Progress bar
        evaluate_bar = tqdm(self.data_loaders[mode], desc=f'Epoch {epoch}. {mode.title()} batches')
        ...
        # loop over batches
        for batch_data in evaluate_bar:
            ...
            if self.args.data_loading == 'pickle':
                ...
                ground_truth, seq_list = inputs['abs_pixel_coord'], inputs['seq_list']
                    # batch_data['abs_pixel_coord'].to(self.args.device), \
                    # batch_data['seq_list'].to(self.args.device)
                inputs['scene'] = evaluate_bar.iterable.dataset.scenes[batch_data['scene_id'].item()]
                batch_id = None
            ...
            all_output, all_aux_outputs = self.net.forward(
                inputs, num_samples=1, if_test=True)

            if mode == 'test':
                # TODO Implement visualization of GT vs PRED
                import random
