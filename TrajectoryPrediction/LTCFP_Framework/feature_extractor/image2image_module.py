...
class Image2ImageModule(pl.LightningModule):
    def __init__(
        ...
        # summary(self.net.to('cuda:0'), (3, 800, 1400), device='cuda') # IMPORTANT: INCLUDE non-Instance of torch.Tensor exclusion, otherwise exception
        # print('\n\n##########################################\n\n')
        # print(self.net)
        # output_tensor = self.net(torch.rand((2,3,800,1400)).to('cuda:0'))   #   ([2, 256, 800, 800])
    ...
    def training_step(self, batch, batch_idx: int):
        ...
        if self.mode == 'img2img':
            train_loss = self.loss_fct(traj_pred.squeeze(), img.float())
        else:
    ...
    def validation_step(self, batch, batch_idx: int) -> None:
        ...
        if self.mode == 'img2img':
            val_loss = self.loss_fct(traj_pred.squeeze(), img.float())
        else:
            raise NotImplementedError('Mode not implemented!')
