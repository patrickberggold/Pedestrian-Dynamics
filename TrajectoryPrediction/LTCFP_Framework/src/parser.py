...
def get_parser(mode, arch, conf):

    if mode == 'train':
        learning_rate = conf[mode][arch]['learning_rate']
        num_epochs = conf[mode][arch]['num_epochs']
        validate_every = conf[mode][arch]['validate_every']
        load_checkpoint = None
    elif mode == 'test':
        load_checkpoint = conf[mode][arch]['load_checkpoint']
        learning_rate = 1e-4
        num_epochs = 300
        validate_every = 20
    model_name = arch
    phase = conf[mode][arch]['phase']
    batch_size = conf[mode][arch]['batch_size']
    device = conf['device']
    ...
    ##############################################################
    parser.add_argument(
        '--dataset', '-d', default=dataset, type=str,
        choices=['eth5', 'sdd', 'ind'],
        help='Dataset selection: ETH-UCY, Stanford Drone, Intersection Drone, '
             'Forking Path')
    parser.add_argument(
        '--test_set', '-ts', default=dataset if not dataset.startswith('eth') else 'eth', type=str,
    ...
    parser.add_argument(
        '--model_name', '-mn', default=model_name, type=str,
        choices=['SAR',
                 'Goal_SAR'],
    ...
    parser.add_argument(
        '--num_epochs', '-ne', default=num_epochs, type=int)
    parser.add_argument(
        '--batch_size', '-bs', default=batch_size, type=int,
    ...
    parser.add_argument(
        '--validate_every', '-ve', default=validate_every, type=int,
    ...
    ##############################################################
    parser.add_argument(
        '--learning_rate', '-lr', default=learning_rate, type=float)
    parser.add_argument(
    ...
    parser.add_argument(
        '--device', default=f"cuda:{device}", type=str, help='What device to use')
    parser.add_argument(
    ...
    ##############################################################
    parser.add_argument(
        '--use_wandb', default=False, type=str2bool, const=True, nargs='?')
    parser.add_argument(
...
def check_and_add_additional_args(args):
    ...
    # set directories
    args.base_dir = 'TrajectoryPrediction/LTCFP_Framework'  # base directory
...
def main_parser(mode, arch, conf, save_config = False, load_config = False):
    """
    Pipeline from_parsed args to args
    """
    # Parse input parameters
    parser = get_parser(mode, arch, conf)
    ...
    if save_config and not os.path.exists(parsed_args.config):
        save_args(parsed_args)
    args = load_args(parser, parsed_args) if load_config else parsed_args
