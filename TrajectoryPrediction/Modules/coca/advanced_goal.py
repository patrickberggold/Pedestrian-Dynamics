...
from helper import visualize_trajectories, visualize_3d
import random
from Datamodules.FloorplanDataset import create_window
from Datamodules.DatasetHelper import create_occupancy_map_torch
from transformers.models.bert.modeling_bert import BertConfig, BertLayer, BertEncoder
...
class AdvancedGoal(nn.Module):
    def __init__(
        ...
        self.real_length_m, self.real_width_m = (800/self.resize_factor+self.pad_size, 1280/self.resize_factor+self.pad_size)

        self.generate_egocentric_every_t = True
        # positional encoding
        ...
        # self.sequence_encoder = BertEncoder(BertConfig(hidden_size=dim, num_hidden_layers=num_enc_layers, num_attention_heads=heads, intermediate_size=dim*ff_mult, hidden_dropout_prob=0.1, attention_probs_dropout_prob=0.1))

        # self.obsNetwork = nn.Sequential(nn.Linear(self.num_obs_steps-1, 128), nn.LeakyReLU(0.2), nn.Linear(128, 128), nn.LeakyReLU(0.2), nn.Linear(128, 1))
        
        self.semantic_dim_reduction = nn.Linear(512, 256)
        self.egocentric_dim_incr = nn.Linear(128, 256)
        ...
        self.fusion = fuse_option
        # option 1: velocity-image cross attention
        if self.fusion == 1:
            # self.obsNetwork = nn.Sequential(nn.Linear(self.num_obs_steps-1, 64), nn.ReLU(), nn.Linear(64, 64), nn.ReLU(), nn.Linear(64, 1))
            self.obsNetwork = nn.Sequential(nn.Linear(self.num_obs_steps-1, 128), nn.LeakyReLU(0.2), nn.Linear(128, 128), nn.LeakyReLU(0.2), nn.Linear(128, 1))
            ...
            self.forward_fusion = self.forward_1
        # option 2: velocity-image concatenation
        elif self.fusion == 2:
            # self.obsNetwork = nn.Sequential(nn.Linear(self.num_obs_steps-1, 64), nn.LeakyReLU(0.2), nn.Linear(64, 64), nn.LeakyReLU(0.2), nn.Linear(64, 1))
            # self.obsNetwork2 = nn.Sequential(nn.Linear(self.num_obs_steps-1, 64), nn.LeakyReLU(0.2), nn.Linear(64, 64), nn.LeakyReLU(0.2), nn.Linear(64, 64))
            # self.dim_mlp = nn.Sequential(nn.Linear(dim, 256), nn.LeakyReLU(0.2), nn.LayerNorm(256), nn.Linear(256, 128), nn.LeakyReLU(0.2), nn.LayerNorm(128), nn.Linear(128, 1))
            self.obsNetwork = nn.Sequential(nn.Linear(self.num_obs_steps-1, 128), nn.LeakyReLU(0.2), nn.Linear(128, 128), nn.LeakyReLU(0.2), nn.Linear(128, 1))
            self.obsNetwork2 = nn.Sequential(nn.Linear(self.num_obs_steps-1, 128), nn.LeakyReLU(0.2), nn.Linear(128, 128), nn.LeakyReLU(0.2), nn.Linear(128, 64))
            self.dim_mlp = nn.Sequential(nn.Linear(dim, 512), nn.LeakyReLU(0.2), nn.LayerNorm(512), nn.Linear(512, 256), nn.LeakyReLU(0.2), nn.LayerNorm(256), nn.Linear(256, 1))
            self.dim_mlp2 = nn.Linear(2*64, dim)
            self.forward_fusion = self.forward_2
        # option 3: velocity-image self-attention
        elif self.fusion == 3:
            self.obsNetwork = nn.Sequential(nn.Linear(self.num_obs_steps-1, 64), nn.LeakyReLU(0.2), nn.Linear(64, 64), nn.LeakyReLU(0.2), nn.Linear(64, 64))
            self.attention_encoder = nn.TransformerEncoder(nn.TransformerEncoderLayer(d_model=dim, nhead=heads, dim_feedforward=dim*ff_mult), num_layers=num_enc_layers)
            self.attn_reduction = nn.Linear(128, 1)
            self.forward_fusion = self.forward_3
    ...
    def forward_1(self, vel_embeds, egocentric_view, destination_view):
        vel_embeds1 = self.obsNetwork(vel_embeds.permute(0,2,1)).squeeze(-1)
        map_view = torch.cat([egocentric_view, destination_view], dim=-1)
        vel_embeds2 = self.decoder_cross_attn(vel_embeds1.unsqueeze(0), map_view.permute(1,0,2)).squeeze(0)
        vel_embeds = vel_embeds1 + vel_embeds2
        return vel_embeds
    
    def forward_2(self, vel_embeds, egocentric_view, destination_view):
        map_view = torch.cat([egocentric_view, destination_view], dim=-1)
        vel_embeds1 = self.obsNetwork(vel_embeds.permute(0,2,1)).squeeze(-1)
        vel_embeds2 = self.obsNetwork2(vel_embeds.permute(0,2,1)).permute(0,2,1)
        cat_vector = torch.cat([vel_embeds2, map_view], dim=1)
        cat_vector = self.dim_mlp(cat_vector).squeeze(-1)
        vel_embeds2 = self.dim_mlp2(cat_vector)#.squeeze(-1)
        vel_embeds = vel_embeds1 + vel_embeds2
        return vel_embeds
        
    def forward_3(self, vel_embeds, egocentric_view, destination_view):
        map_view = torch.cat([egocentric_view, destination_view], dim=-1)
        vel_embeds = self.obsNetwork(vel_embeds.permute(0,2,1)).permute(0,2,1)
        attn_vector = torch.cat([vel_embeds, map_view], dim=1).permute(1,0,2)
        vel_embeds = self.attention_encoder(attn_vector).permute(1,2,0)
        vel_embeds = self.attn_reduction(vel_embeds).squeeze(-1)
        return vel_embeds
    ...
    def forward(
        ...
        # abs_coordinates_r = transform_to_real(abs_coordinates, image.shape[-2:])
        ...
        obs_velocities = obs_coords[:, 1:] - obs_coords[:, :-1]
        # vel_embeds = self.masked_self_attention(obs_velocities, num_agents)
        vel_embeds = self.coord_emb(obs_velocities)
        vel_embeds = self.sequence_encoder(vel_embeds.permute(1,0,2)).permute(1,0,2) # if not self.use_bert else self.sequence_encoder(vel_embeds).last_hidden_state

        # img information
        _, egocentric_view = self.img_encoder(occupancyMaps_egocentric, mode=1)
        egocentric_view = egocentric_view.flatten(-2).permute(0,2,1)
        egocentric_view = self.egocentric_dim_incr(egocentric_view)

        _, destination_view = self.img_encoder(semanticMaps_per_agent, mode=0)
        destination_view = destination_view.flatten(-2).permute(0,2,1)
        destination_view = self.semantic_dim_reduction(destination_view)

        vel_embeds = self.forward_fusion(vel_embeds, egocentric_view, destination_view)
        ...
        # TODO Decoders: TF decoder for teacher forcing, VAE, GAN
        pred_coords = []
        ...
        for i_step in range(self.num_obs_steps, self.seq_length):
            ...
            if num_future_steps > 1:
                if self.generate_egocentric_every_t:
                    _, global_occupancy_map = create_occupancy_map_torch(semanticMaps_per_agent.shape[-2:], last_pos, resize_factor=1.0, gaussian_std=self.gaussian_std)
                    egocentric_view_t = create_egocentric_maps(num_agents, semanticMaps_per_agent, global_occupancy_map, last_pos, self.window_size)
                    _, egocentric_view_t = self.img_encoder(egocentric_view_t, mode=1)
                    egocentric_view = self.egocentric_dim_incr(egocentric_view_t.flatten(-2).permute(0,2,1))

                # vel_embeds = self.masked_self_attention(entire_predicted_sequence[:, 1:]-entire_predicted_sequence[:,:-1], num_agents)
                vel_embeds = self.coord_emb(entire_predicted_sequence[:, 1:]-entire_predicted_sequence[:,:-1])
                vel_embeds = self.sequence_encoder(vel_embeds.permute(1,0,2)).permute(1,0,2) # if not self.use_bert else self.sequence_encoder(vel_embeds).last_hidden_state
                vel_embeds = self.forward_fusion(vel_embeds, egocentric_view, destination_view)

            # coordinate decoding
            out = self.to_logits(vel_embeds) if vel_embeds.shape[0]==num_agents else self.to_logits(vel_embeds.permute(1,0,2))
            outputs_current = out#[-num_agents:]#.clone()#.detach()
            
            # for velocitites specifically
        ...
        out_dir.update({'losses_mse': losses_mse})
        pred_coords_cl = pred_coords.clone().detach()
        label_coords_cl = label_coords.clone().detach()
    ...
    def compute_loss(self, prediction, batch, stage, save_path=None):
        ...
        if save_path is not None:
            abs_coordinates_draw = abs_coordinates.clone()
            pred_coords_draw = prediction['pred_coords'].clone().detach()
    ...
    def compute_mse_and_ade(self, outputs, ground_truth, last_obs_gt):
        ...
        ground_truth_clone = ground_truth.clone().detach()
...
def create_egocentric_maps(num_agents, semanticMaps_per_agent, global_occupancy_map, agent_coords, window_size):
    occupancyMaps_egocentric_list = []
    for i in range(num_agents):
        globalSemMap_i = semanticMaps_per_agent[i, 0, :, :]
        assert globalSemMap_i.shape == global_occupancy_map.shape
        globalSemMap_i[globalSemMap_i == 0] = global_occupancy_map[globalSemMap_i == 0]
        # visualize_3d(globalSemMap_i)
        x_coord_i, y_coord_i = agent_coords[i, 1], agent_coords[i, 0]
        if torch.isfinite(x_coord_i):
            x_coord_i, y_coord_i = int(x_coord_i), int(y_coord_i)
            mid_value = globalSemMap_i[x_coord_i, y_coord_i].item()
            if abs(mid_value) != 1.0:
                ...
                mid_value_neighbor_1, mid_value_neighbor_2, mid_value_neighbor_3, mid_value_neighbor_4 = globalSemMap_i[x_coord_i+1, y_coord_i].item(), globalSemMap_i[x_coord_i-1, y_coord_i].item(), globalSemMap_i[x_coord_i, y_coord_i+1].item(), globalSemMap_i[x_coord_i, y_coord_i-1].item()
            ...
            window = create_window(globalSemMap_i, (x_coord_i, y_coord_i), window_size//2) # globalSemMap_i[(x_coord_i-window_size//2):(x_coord_i+window_size//2), (y_coord_i-window_size//2):(y_coord_i+window_size//2)]
        ...
        occupancyMaps_egocentric_list.append(window)
    return torch.stack(occupancyMaps_egocentric_list, dim=0).unsqueeze(1)  


def transform_to_real(pix_coords, img_shape):
    # do this transformation to real coordinates again....
    or_length, or_width = 320, 400
    resize_factor = 2
    max_length, max_width = img_shape
    pad_on_max = max(max_length, max_width) - or_width
    pad_on_min = min(max_length, max_width) - or_length

    if max_length < max_width:
        assert torch.max(pix_coords[:,:,1]).item() < max_length, 'coordinates must be within the image!' # dim 1 is minimum
        subtr_tensor = torch.tensor([[pad_on_max/2, pad_on_min/2]]).expand(pix_coords.shape[0], pix_coords.shape[1], 2).to(pix_coords.device)
        abs_coordinates_r = (pix_coords.clone() - subtr_tensor) / (32 / resize_factor)
        assert torch.max(abs_coordinates_r).item() > 0 and torch.max(abs_coordinates_r[:,:,1]).item() < 20 and torch.max(abs_coordinates_r[:,:,0]).item() < 25
    else:
        assert torch.max(pix_coords[:,:,0]).item() < max_width, 'coordinates must be within the image!' # dim 0 is minimum
        subtr_tensor = torch.tensor([[pad_on_min/2, pad_on_max/2]]).expand(pix_coords.shape[0], pix_coords.shape[1], 2).to(pix_coords.device)
        abs_coordinates_r = (pix_coords.clone() - subtr_tensor) / (32 / resize_factor)
        assert torch.max(abs_coordinates_r).item() > 0 and torch.max(abs_coordinates_r[:,:,0]).item() < 20 and torch.max(abs_coordinates_r[:,:,1]).item() < 25
    # transformation end...
    return abs_coordinates_r
