...
class CoCaGoal(nn.Module):
    def __init__(
        ...
        coords_normed=True, separate_agent_batches=False, fuse_option=1
    ):
        ...
        self.separate_agent_batches = separate_agent_batches
        ...
        self.context_emb_occMap = nn.Linear(dim//4, dim//4)
        # train ADE: sepTrue_fuse3, sepTrue_fuse1, sepTrue_fuse2, sepFalse_fuse1, sepFalse_fuse3, sepFalse_fuse2
        # val ADE: sepTrue_fuse3, sepTrue_fuse1, sepTrue_fuse2, sepFalse_fuse1, sepFalse_fuse3, sepFalse_fuse2

        # load the image encoder
        self.img_encoder = ResNet18Adaption()
        if self.pretrained_vision:
            ckpt_path = os.sep.join(['TrajectoryPrediction', 'Modules', 'coca', 'checkpoints', 'Resnet18Adaption_pretrained_lr3e-4_withTanh'])
    ...
    def forward(
        ...
        if self.separate_agent_batches:
        ...
        # mean over obs timesteps, then Linear() --> z, mu, logvar (per agent) with dim_z = 32
        ...
        # TODO: build in the optional goal support here, how imprtant is it for performance?
        ...
        context_vector = torch.cat((context_emb_obs_sem, context_egoOccMap, context_goals), dim=-1)
        ...
        # TODO for an extension: max_step_size, egocentric heatmaps every timestep, intermediate goal predictor
        pred_coords = []
        # decode
        for i_step in range(self.num_obs_steps, self.seq_length):
            ...
            if self.separate_agent_batches:
                ...
                assert pos_enc.shape == decoder_emb.shape and decoder_emb.shape == context_vector_rep.shape
                decoder_emb += pos_enc
            else:
            ...
            # mask shape check
            T_len, S_len, bs = decoder_emb.shape[0], context_vector_rep.shape[0], decoder_emb.shape[1]
            assert tgt_mask.shape == (bs*self.heads, T_len, S_len) and decoder_emb.shape[1]==context_vector_rep.shape[1], 'incorrect shapes!'
            
            # cross-attention between dec_in and context
            decoder_out = self.decoder_cross_attn(decoder_emb, context_vector_rep, tgt_mask=tgt_mask, memory_mask=memory_mask)
            ...
            outputs_current = out[-num_agents:].detach().clone()
            if self.coords_normed:
    ...
    def compute_loss(self, prediction, batch, stage, save_path=None):
        ...
        # metrics calculation        
        # prediction_cl = prediction.clone().detach()
        # labels_cl = label_coords.clone().detach()            
        # if self.coords_normed:
        #     prediction_cl, labels_cl = self.denormalize([prediction_cl, labels_cl], onlyStd=True)

        # ade_loss = torch.sum((torch.abs(prediction_cl - labels_cl).masked_fill(~is_traj_mask, 0)))  / torch.sum(is_traj_mask)
        metrics = {'ADE_true_caption': ade_loss.item(), 'FDE_true_caption': fde_loss.item()}
