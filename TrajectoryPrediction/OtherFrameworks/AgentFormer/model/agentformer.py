...
class ContextEncoder(nn.Module):
    def __init__(self, cfg, ctx, **kwargs):
        ...
        if 'map' in self.input_type:
            in_dim += ctx['map_enc_dim'] - self.motion_dim # TODO change the out dims of the backbone from 15x1 -> 7x7 maybe
...
class AgentFormer(nn.Module):
    def __init__(self, cfg):
        ...
        self.map_global_rot = cfg.get('map_global_rot', True) # changed
        self.ctx['input_type'].append('map') # changed
        ...
        if self.use_map:
            ...
            dict_list = []
            # TODO backbone needs to be retrained probably again
            for key, tensor in torch.load(CKPT_PATH)['state_dict'].items():
                if key.startswith('backbone'):
                    dict_list.append((key.replace('backbone', 'net.0'), tensor))
                # elif key.startswith('reconstruction'):
                #     dict_list.append((key.replace('reconstruction', 'net.3'), tensor))
            state_dict = OrderedDict(dict_list)
            ...
            model = Image2ImageModule(mode='img2img', relu_at_end=False)
            ...
            model.net.eval()
            self.map_encoder = model

            self.ctx['map_enc_dim'] = 15*15
    ...
    def set_data(self, data):
        ...
        # img = np.array(data['scene_map'])
        # plt.imshow(img)
        # img = torch.from_numpy(img).unsqueeze(0).float()
        # img = img.swapaxes(3,1).swapaxes(2,3).to(self.device)
        # pred = self.map_encoder.forward(img)
        ...
        if self.use_map:
            ...
            self.data['agent_maps'] = scene_map.get_cropped_maps(scene_points, patch_size, rot).to(device) # mini maps centering every agent 100x100
