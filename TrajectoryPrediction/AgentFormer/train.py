""" Based on https://github.com/Khrylx/AgentFormer """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu', type=int, default=CUDA_DEVICE)
    args = parser.parse_args()

    """ setup """
    args.cfg = "eth_agentformer"
    cfg = Config(args.cfg, args.tmp, create_dirs=True)
    cfg.seed = 42
    cfg.pred_epoch = 0
    cfg.dataset = 'floorplan_simple'
    if cfg.dataset == 'floorplan_simple':
        # adjust min_future_frames (12) and min_past_frames (8) because these account for approx. 0.9/0.6 predicted/observed seconds in the eth dataset
        # the crowdit simulator logs coordinates with 2 fps ==> dt = 0.5 seconds ==> min_future_frames = 2 and min_past_frames = 1 for now...
        # Alternative: linear interpolation between timestamps, but this might make the files very large
        # cfg.min_future_frames = cfg.future_frames  = 2
        # cfg.min_past_frames = cfg.past_frames = 1
        cfg.load_map = True
        # cfg.use_map = True
