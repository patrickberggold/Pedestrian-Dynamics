...
class DeepLabHead(nn.Sequential):
    def __init__(self, in_channels: int, num_classes: int, segment: bool = True) -> None:
        super().__init__(
            ASPP(in_channels, [12, 24, 36], segment=segment),
...
# class DeepLabRegressionHead(nn.Sequential):
...
class ASPPPooling(nn.Sequential):
    def __init__(self, in_channels: int, out_channels: int, segment: bool = True) -> None:
        super().__init__(
...
class ASPP(nn.Module):
    def __init__(self, in_channels: int, atrous_rates: List[int], out_channels: int = 256, segment: bool = True) -> None:
        super().__init__()
        modules = []
        modules.append(
        ...
        modules.append(ASPPPooling(in_channels, out_channels, segment))
...
def _deeplabv3_resnet(
    ...
    if not img2img:
        classifier = DeepLabHead(2048, num_classes, segment)
    else:
        if rgb_img:
            classifier = DeepLabHead(2048, 3)
        else:
...
def deeplabv3_resnet50(
    ...
    model = _deeplabv3_resnet(backbone, num_classes, aux_loss, segment, img2img, rgb_img)
