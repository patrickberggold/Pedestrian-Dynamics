...
import cv2
import glob
from .map_floorplan import GeometricMap
import h5py

class preprocess(object):
    
    def __init__(self, data_root, seq_name, parser, log, img_name=None, split='train', phase='training'):
        self.parser = parser
        ...
        if parser.dataset == 'nuscenes_pred':
            ...
            label_path = f'{seq_name}'
            delimiter = ','
        else:
            assert False, 'error'

        self.gt = np.genfromtxt(label_path, delimiter=delimiter, dtype=str)
        if not parser.dataset in {'eth', 'hotel', 'univ', 'zara1', 'zara2', 'nuscenes_pred'}:
            # convert crowdit timestamps to frames
            self.gt[:, 0] = self.gt[:, 0].astype(np.float32)*2.-1
        ...
        if self.load_map:
            assert img_name
            assert int(img_name.split('\\')[-2].split('__')[0]) ==  int(seq_name.split('\\')[-2].split('__')[0])
            assert int(img_name.split('variation_')[-1].replace('.h5', '')) == int(seq_name.split('variation_')[-1].replace('.txt', ''))
            self.load_scene_map(img_name)
        else:
        ...
        # assign class 'Pedestrian'
        if parser.dataset in {'eth', 'hotel', 'univ', 'zara1', 'zara2', 'nuscenes_pred'}:
            for row_index in range(len(self.gt)):
                self.gt[row_index][2] = 1
        else:
            self.gt = np.insert(self.gt, 2, np.ones(self.gt.shape[0]), axis=1) 
        
        self.gt = self.gt.astype('float32')
        self.xind, self.zind = 3, 4
    ...
    def PreData(self, frame):
        # get all data from previous frames (frame, id, x, y)
        DataList = []
    ...
    def FutureData(self, frame):
        # get all data from successive frames (frame, id, x, y)
        DataList = []
    ...
    def load_scene_map(self, map_location):
        ...
        # read hdf5 file
        self.scene_map = np.array(h5py.File(map_location, 'r').get('img'))
        self.scene_map = np.moveaxis(self.scene_map, -1, 0)
        self.geom_scene_map = GeometricMap(self.scene_map)
        ...
        for identity in valid_id:
            ...
            for j in range(self.past_frames):
                ...
                if len(past_data) > 0 and identity in past_data[:, 1]:
                    found_data = past_data[past_data[:, 1] == identity].squeeze()[[self.xind, self.zind]] / self.past_traj_scale # TODO what is this past_traj_scale?
    ...
    def __call__(self, frame):
        ...
        valid_id = self.get_valid_id(pre_data, fut_data) # filter all agents that are present in all previous and future frames
        ...
        pre_motion_3D, pre_motion_mask = self.PreMotion(pre_data, valid_id) # returns previous scaled 2D coordinates and masks of valid agents
        fut_motion_3D, fut_motion_mask = self.FutureMotion(fut_data, valid_id) # returns successive scaled 2D coordinates and masks of valid agents
