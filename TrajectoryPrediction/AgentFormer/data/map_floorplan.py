"""
Code based on Trajectron++: https://github.com/StanfordASL/Trajectron-plus-plus/blob/ef0165a93ee5ba8cdc14f9b999b3e00070cd8588/trajectron/environment/map.py
"""
...
class GeometricMap(Map):
    ...
    """
    def __init__(self, data, origin=None, description=None):
        #assert isinstance(data.dtype, np.floating), "Geometric Maps must be float values."
        super(GeometricMap, self).__init__(data, description=description)
    ...
    def get_cropped_maps_from_scene_map_batch(cls, maps, scene_pts, patch_size, rotation=None, device='cpu'):
        ...
        centers = torch.tensor(scene_pts, dtype=torch.long, device=device) \
            + torch.tensor([context_padding_x, context_padding_y], device=device, dtype=torch.long)
    ...
    """ Project pedestrians coordinates into map frame """
    """ I dont think this is necessary because here the pedestrian coordinates are absolute and taken from a bird view-perspective """
    """ Actually I may need this for mapping coords -> pixel coords """
    def to_map_points(self, scene_pts):
        org_shape = None
