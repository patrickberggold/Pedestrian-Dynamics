...
from ..anchors import AnchorLabeler, Anchors
from timm.data.distributed_sampler import OrderedDistributedSampler
import os

MAX_NUM_INSTANCES = 20 # default in EfficientDet: 100
...
class DetectionFastCollate:
    ...
    def __init__(
        ...
        self.anchor_labeler = AnchorLabeler(
            Anchors(model_config.min_level, model_config.max_level, model_config.num_scales, model_config.aspect_ratios, model_config.anchor_scale, config['img_max_size']),
            config['num_classes'],
            match_threshold=0.5,
        )
        self.mode = mode
    ...
    def __call__(self, batch):
        ...
        img_tensor = torch.stack([torch.from_numpy(b[0]) for b in batch], dim=0).float()
        ...
        if self.mode==1:
            ...
            for batchItem in batch:
                labels.append(torch.from_numpy(batchItem[1]['cls']))
                bboxes.append(torch.from_numpy(batchItem[1]['bbox']))
            target.update({'gt_targets': [labels, bboxes]})
        
        return img_tensor, target, torch.LongTensor([b[2] for b in batch])
