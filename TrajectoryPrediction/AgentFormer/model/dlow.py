...
class DLow(nn.Module):
    ...
    def main(self, mean=False, need_weights=False):
        ...
        if hasattr(pred_model, 'use_map') and pred_model.use_map:
            # TODO technically this needs to be changed because the output dims are completely off (tighter bottleneck is needed), 
            # but generally maybe a different architecture is needed because this doesnt use global information, only local, agent-centered information
